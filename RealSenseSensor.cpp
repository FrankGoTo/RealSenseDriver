...
void RealSenseSensor::createFirstConnected()
{
	...
	pipe_config.enable_stream(RS2_STREAM_COLOR, color_width, color_height, RS2_FORMAT_RGB8, frame_rate);
...
bool RealSenseSensor::processDepth()
{
	...
	if (pipe.poll_for_frames(&frames)){
		frames = align_to_depth.process(frames);//align to depth view
